...
if($_POST["userName"] != ""){
	...
	if ($regusrs > 0) {
		...
		$query = $db->prepare("INSERT INTO accounts (userName, password, email, isAdmin, registerDate, isActive, gjp2)
		VALUES (:userName, :password, :email, :isAdmin, :time, :isActive, :gjp)");
		$query->execute([':userName' => $userName, ':password' => $hashpass, ':email' => $email, ':isAdmin' => 1, ':time' => time(), ':isActive' => $preactivateAccounts ? 1 : 0, ':gjp' => $gjp2]);
		echo "1";
	}
}
?>
