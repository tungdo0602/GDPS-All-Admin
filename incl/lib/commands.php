...
class Commands {
	public static function ownCommand($comment, $command, $accountID, $targetExtID){
		...
		if(substr($comment,0,$commandlength) == $commandInComment){
			return true;
		}
		return false;
	}
	public static function doCommands($accountID, $comment, $levelID) {
		...
		if(substr($comment,0,5) == '!like'){
			$like = $commentarray[1];
			if($like != ""){
				$query = $db->prepare("UPDATE levels SET likes=$like WHERE levelID = :levelID");
				$query->execute([':levelID' => $levelID]);
				$query = $db->prepare("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES ('2', :value, :levelID, :timestamp, :id)");
				$query->execute([':value' => "0", ':timestamp' => $uploadDate, ':id' => $accountID, ':levelID' => $levelID]);
			}
		...
		if(substr($comment,0,9) == '!download'){
			$download = $commentarray[1];
			if($download != ""){
				$query = $db->prepare("UPDATE levels SET downloads=$download WHERE levelID = :levelID");
				$query->execute([':levelID' => $levelID]);
				$query = $db->prepare("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES ('2', :value, :levelID, :timestamp, :id)");
				$query->execute([':value' => "0", ':timestamp' => $uploadDate, ':id' => $accountID, ':levelID' => $levelID]);
			}
		...
		if(substr($comment,0,11) == '!coinamount'){
			$cm = $commentarray[1];
			if($cm != ""){
				$query = $db->prepare("UPDATE levels SET coins=$cm WHERE levelID = :levelID");
				$query->execute([':levelID' => $levelID]);
				$query = $db->prepare("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES ('2', :value, :levelID, :timestamp, :id)");
				$query->execute([':value' => "0", ':timestamp' => $uploadDate, ':id' => $accountID, ':levelID' => $levelID]);
			}
